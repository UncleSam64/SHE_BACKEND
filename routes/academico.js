...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../uploads/academico'));
  },
...
router.post('/upload', upload.single('excelFile'), async (req, res) => {
  ...
  const folder = path.join(__dirname, '../uploads/academico');
 ...
 // Crear objetos JSON con los datos del archivo
    ...
    res.status(200).json({ mensaje: 'Archivo académico subido y datos insertados correctamente.' });
    ...
    res.status(500).send('Error al procesar e insertar datos académicos.');
  }
});
